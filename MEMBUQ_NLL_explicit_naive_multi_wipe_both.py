...
from qdax.core.map_elites_MBUQ_NLL_explicit_naive_both import MAPElites
from qdax.tasks.arm import arm_scoring_function, noisy_arm_scoring_function, both_scoring_function
from qdax.utils.metrics import CSVLogger, default_qd_metrics_UQ
from qdax.utils.plotting import plot_map_elites_results_UQ
from qdax.core.stochasticity_utils_UQ import reevaluation_function_both, sampling_both
...
scoring_functions = {
    ...
    "both": functools.partial( # Here
        both_scoring_function,
        fit_variance=0.01, 
        desc_variance=0.05, 
        params_variance=0., 
    ), 
...
def test_arm(
    ...
    OT_function_predictions_arm = []
    OT_function_predictions_noisy_arm = []
    ...
    for i in range(num_iterations):
        ...
        if i <= model_start_training:
            ...
            repertoire, genotypes, fitnesses_arm, fitnesses_var_arm, descriptors_arm, descriptors_var_arm, fitnesses_noisy_arm, fitnesses_var_noisy_arm, descriptors_noisy_arm, descriptors_var_noisy_arm, emitter_state, metrics, random_key = map_elites.update_buffer_beginning(
            ...
            buffer = replay_buffer.add_transitions_explicit(
                buffer=buffer,
                fitnesses_var=fitnesses_var_noisy_arm,
                descriptors_var=descriptors_var_noisy_arm,
                genotypes=genotypes,
                fitnesses=fitnesses_noisy_arm,
                bd1=descriptors_noisy_arm[:,0],
                bd2=descriptors_noisy_arm[:,1]
            )
        ...
        # After the first generations, buffer via scoring function and container via model 
            ...
            genotypes, fitnesses_arm, fitnesses_var_arm, descriptors_arm, descriptors_var_arm, fitnesses_noisy_arm, fitnesses_var_noisy_arm, descriptors_noisy_arm, descriptors_var_noisy_arm, random_key = map_elites.generate_batch(
                repertoire=repertoire,
                emitter_state=emitter_state,
                random_key=random_key,
            )
            ...
            for subdiv in range(nb_subdiv):
                ...
                ### Producing genotypes and associated scores with the scoring function
                # subdiv_genotypes, subdiv_fitnesses, subdiv_fitnesses_var, subdiv_descriptors, subdiv_descriptors_var, random_key
                subdiv_genotypes, subdiv_fitnesses_arm, subdiv_fitnesses_var_arm, subdiv_descriptors_arm, subdiv_descriptors_var_arm, subdiv_fitnesses_noisy_arm, subdiv_fitnesses_var_noisy_arm, subdiv_descriptors_noisy_arm, subdiv_descriptors_var_noisy_arm, random_key = map_elites.generate_batch(
                    repertoire=repertoire,
                    emitter_state=emitter_state,
                    random_key=random_key,
                )
            ...
            subdiv_mean_fitness, subdiv_mean_bd1, subdiv_mean_bd2, subdiv_std_fitness, subdiv_std_bd1, subdiv_std_bd2 = map_elites.model_batch_predict_2(
                batch_size=batch_size,
                model_dict=model_state,
                genotypes=list_geno,
            )

            subdiv_mean_bd = jnp.stack((subdiv_mean_bd1, subdiv_mean_bd2), axis=1)
            subdiv_std_bd = (subdiv_std_bd1 + subdiv_std_bd2) / 2
            ...
            repertoire, emitter_state, metrics = map_elites.update_via_model(
                ...
                emitter_state=emitter_state,
                genotypes=list_geno,
                fitnesses=subdiv_mean_fitness,
                descriptors=subdiv_mean_bd,
                fitnesses_var=subdiv_std_fitness,
                descriptors_var=subdiv_std_bd,
                extra_scores=None,
            )
        ...
        if (i != 0 and i % freq_training == 0 and i - model_start_training > 0) or i == model_start_training or i == num_iterations-1:
            ...
            if check_wipe == 1:
                ...
                ### Predicting scores with the model
                new_mean_fitness, new_mean_bd1, new_mean_bd2, new_std_fitness, new_std_bd1, new_std_bd2 = map_elites.model_batch_predict_2(
                    batch_size=batch_size,
                    model_dict=model_state,
                    genotypes=genotypes_in_repertoire,
                )

                new_mean_bd = jnp.stack((new_mean_bd1, new_mean_bd2), axis=1)
                new_std_bd = (new_std_bd1 + new_std_bd2) / 2

                repertoire, emitter_state, metrics = map_elites.update_via_model(
                    repertoire=repertoire,
                    emitter_state=emitter_state,
                    genotypes=genotypes_in_repertoire,
                    fitnesses=new_mean_fitness,
                    descriptors=new_mean_bd,
                    fitnesses_var=new_std_fitness,
                    descriptors_var=new_std_bd,
                    extra_scores=None,
                )
        ...
        # to_predict generation before training, the NN uses the repertoire to predict some scores 
            ...
            preds_func_arm = jnp.concatenate((fitnesses_arm.reshape((-1,1)), descriptors_arm, fitnesses_var_arm.reshape((-1,1)), descriptors_var_arm.reshape((-1,1))), axis=1)
            preds_func_noisy_arm = jnp.concatenate((fitnesses_noisy_arm.reshape((-1,1)), descriptors_noisy_arm, fitnesses_var_noisy_arm.reshape((-1,1)), descriptors_var_noisy_arm.reshape((-1,1))), axis=1)
            ...
            OT_function_predictions_arm.append(preds_func_arm)
            OT_function_predictions_noisy_arm.append(preds_func_noisy_arm)
        ...
        if i%log_period == 0:
            ...
            if check_wipe == 1:
                ...
                if (i != 0 and i % freq_training == 0 and i - model_start_training > 0) or i == model_start_training or i == num_iterations-1:
                    ...
                    (reeval_repertoire, _, _, _, _, random_key) = reevaluation_function_both(
    ...
    if check_wipe == 1:
        ...
        fig, axes = plot_map_elites_results_UQ(env_steps=env_steps, metrics=all_metrics, repertoire=old_repertoire, min_bd=min_bd, max_bd=max_bd)
    else:
        # create the plots and the grid with corresponding uncorrected metric and final repertoire
    ...
    OT_function_predictions_arm = jnp.concatenate(OT_function_predictions_arm) 
    OT_function_predictions_noisy_arm = jnp.concatenate(OT_function_predictions_noisy_arm) 
    ...
    jnp.save(f'{full_path}/{method}_{task_name}_OT_function_predictions_arm_{time_format}.npy', OT_function_predictions_arm)
    jnp.save(f'{full_path}/{method}_{task_name}_OT_function_predictions_noisy_arm_{time_format}.npy', OT_function_predictions_noisy_arm) 
...
def specify_arg():
    ...
    parser.add_argument("--algo_name", default="MEMBUQ_NLL_Explicit_Naive_Both", type=str, help="Name of the algorithm")
    ...
    parser.add_argument("--sub_div", default=100, type=int, help="Number of batch of genotypes the model will predict at each generation")
