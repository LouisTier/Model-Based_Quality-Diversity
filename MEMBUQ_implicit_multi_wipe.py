...
from qdax.core.map_elites_MBUQ_implicit import MAPElites
...
def test_arm(
    ...
    model_start_training = first_train # Number of generations to fill container before training the model: 1000
    freq_training = model_start_training/2 # Number of generations between 2 training except the first one at model_start_training generations: 500

    buffer_size = int(keeping_size * freq_training * batch_size) # Size of the buffer to store genotypes and scores
    num_epochs = num_epochs # Number of epochs at each training: 200 
    ...
    output_size = 6 # Fitness_mean, BD1_mean, BD2_mean, Fitness_std, BD1_std and BD2_std for the NN predictions: 6
    ...
    for i in range(num_iterations):
        ...
        if i <= model_start_training:
            ...
            repertoire, genotypes, fitnesses, descriptors, emitter_state, metrics, random_key = map_elites.update_buffer_beginning(
            ...
            buffer = replay_buffer.add_transitions_implicit(
                buffer=buffer,
                genotypes=genotypes,
                fitnesses=fitnesses,
                bd1=descriptors[:,0],
        ...
        # After the first generations, buffer via scoring function and container via model 
            ...
            genotypes, fitnesses, descriptors, random_key = map_elites.generate_batch(
                repertoire=repertoire,
                emitter_state=emitter_state,
                random_key=random_key,
            )
            ...
            for subdiv in range(nb_subdiv):
                ...
                ### Producing genotypes and associated scores with the scoring function
                subdiv_genotypes, subdiv_fitnesses, subdiv_descriptors, random_key = map_elites.generate_batch(
                    repertoire=repertoire,
                    emitter_state=emitter_state,
                    random_key=random_key,
                )
        ...
        if (i != 0 and i % freq_training == 0 and i - model_start_training > 0) or i == model_start_training or i == num_iterations-1:
            ...
            ### Testing the model 
            val_mean_fitness_mod, val_mean_bd1_mod, val_mean_bd2_mod, val_std_fitness_mod, val_std_bd1_mod, val_std_bd2_mod, val_fit_func, val_bd1_func, val_bd2_func, val_ngll = map_elites.check_model(
                model_dict=model_state,
        ...
        # to_predict generation before training, the NN uses the repertoire to predict some scores 
            ...
            # Model has uncertainties but the environment hasn't so can't really compare
...
def specify_arg():
    ...
    parser.add_argument("--learning_rate", default=0.005, type=float, help="Learning rate of the optimizer (Adam)")
    ...
    parser.add_argument("--algo_name", default="MEMBUQ_Implicit", type=str, help="Name of the algorithm")
    ...
    # To fine tune
    parser.add_argument("--first_train", default=500, type=int, help="Number of the generation when the model is trained for the first time")
    parser.add_argument("--num_epochs", default=225, type=int, help="Number of epochs to train the model") # 200
    parser.add_argument("--per_data", default=0.4, type=float, help="Percentage of old data to be retained for next training session")
...
if __name__ == "__main__":
    ...
    # # python3 MEMBUQ_implicit.py --save_dir test_grid  --task_name arm
