...
from qdax.core.map_elites_UQD import MAPElites
...
from qdax.utils.sampling import sampling as spl

# from qdax.core.stochasticity_utils import sampling as spl
...
# MES_path = os.path.join(result_path, "MES")
# create_directory(MES_path)
# MES_rep_path = os.path.join(MES_path, "repertoires")
# create_directory(MES_rep_path)
# arm_path = os.path.join(MES_rep_path, "arm")
...
# noisy_arm_path = os.path.join(MES_rep_path, "noisy_arm")
...
def test_arm(task_name: str, batch_size: int, nb_seed: int, save_dir: str, time_format: str) -> None:
    ...
    saving_path = "MES_" + task_name + "_" + time_format
    # print(saving_path)
    ...
    # the approximated gradient (each of them is evaluated 8 times)
    ...
    metrics_fn = functools.partial(
        default_qd_metrics_UQ, ### qd_score, max_fitness, coverage
    ...
    for i in tqdm.tqdm(range(num_loops)):
        ...
        if i==0:
            print(f"\n\nBeginning Sampling with {task_name} and {num_iterations} generations with a batch of size {batch_size} at {time_format} \n")
        ...
        (reeval_repertoire, _, _, _, _, random_key)= reevaluation_function(repertoire=repertoire,
                                                                         ...
                                                                         scoring_fn=scoring_fn, #No sampling: don't use spl_fn
                                                                         num_reevals=256
                                                                         )
        ...
        for key, value in metrics.items():
            ...
            if key in all_metrics.keys():
                # print("concatenate: ", all_metrics[key], "and ", value)
        ...
        ### Logging/writing new metrics to the csv qfile every 10 generations (log_period).
    ...
    env_steps = jnp.arange(num_iterations) * batch_size * 8
    ...
    plt.savefig(f'{save_dir}/MES/{saving_path}.png')

    env_steps = jnp.arange(num_iterations/log_period) * batch_size * 8 * log_period # equal
    ...
    ### Saving corresponding figure (3 metrics + final archive)
    plt.savefig(f'{save_dir}/MES/reeval_{saving_path}.png')

    # global_path = f"{save_dir}/MES/repertoires/"
    # # saving_rep_unc = f"results_sim/MES/repertoires/{task_name}_unc_rep"
    # # saving_rep_cor = f"results_sim/MES/repertoires/{task_name}_cor_rep"
...
if __name__ == "__main__":
    ...
    # test_arm(task_name="arm", batch_size=64, nb_seed=42)
    # test_arm(task_name="noisy_arm", batch_size=64, nb_seed=42)
