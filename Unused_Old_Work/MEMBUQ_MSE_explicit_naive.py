...
from qdax.core.map_elites_MBUQ_explicit_naive import MAPElites
...
def test_arm(
    ...
    """
    Run MEMBUQ Explicit Naive algorithm with MSE loss
    ...
    output_size = 5 # Fitness, BD1, BD2, Fitness_Std and BD_Std for the NN predictions: 5
    ...
    # mixing_emitter = MixingEmitter_UQ(
    #     mutation_fn=lambda x, y: (x, y), 
    #     variation_fn=variation_fn, 
    #     variation_percentage=1.0, 
    #     batch_size=batch_size,
    ...
    # for i in range(num_iterations):
        ...
        # After the first generations, buffer via scoring function and container via model 
            ...
            fitness_model, descriptor_model, fitness_var_model, descriptor_var_model = map_elites.model_batch_predict(
            ...
            repertoire, emitter_state, metrics = map_elites.update_via_model(
                    ...
                    fitnesses_var=fitness_var_model,
                    descriptors_var=descriptor_var_model,
                    extra_scores=None,
            )
        ...
        if (i != 0 and i % freq_training == 0 and i - model_start_training > 0) or i == model_start_training or i == num_iterations-1:
            ...
            ### Testing the model
            fit_mod, fit_func, BD_mod, BD_func, fit_var_mod, fit_var_func, BD_var_mod, BD_var_func, val_loss = map_elites.check_model(
                model_dict=model_state,
        ...
        # to_predict generation before training, the NN uses the repertoire to predict some scores 
            ...
            preds_model = jnp.concatenate((fitness_model.reshape((-1,1)), descriptor_model, fitness_var_model.reshape((-1,1)), descriptor_var_model.reshape((-1,1))), axis=1)
    ...
    jnp.save(f'{full_path}/{method}_{task_name}_BD_var_func_{time_format}.npy', BD_var_func)
    jnp.save(f'{full_path}/{method}_{task_name}_BD_var_mod_{time_format}.npy', BD_var_mod)
    jnp.save(f'{full_path}/{method}_{task_name}_fit_var_func_{time_format}.npy', fit_var_func)
    jnp.save(f'{full_path}/{method}_{task_name}_fit_var_mod_{time_format}.npy', fit_var_mod)
...
def specify_arg():
    ...
    parser.add_argument("--num_epochs", default=5, type=int, help="Number of epochs to train the model")
    parser.add_argument("--per_data", default=0.5, type=float, help="Percentage of old data to be retained for next training session")
...
if __name__ == "__main__":
    ...
    # python3 MEMBUQ_explicit_naive.py --save_dir test_grid_UQ --task_name arm --first_train 500 --num_epochs 5 --per_data 0.5
    # python3 MEMBUQ_explicit_naive.py --save_dir results_sim_arg --task_name noisy_arm --first_train 500 --num_epochs 125 --per_data 0.5
    ...
    # # python3 MEMBUQ_explicit_naive.py --save_dir test_grid  --task_name arm
