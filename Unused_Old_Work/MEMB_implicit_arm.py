...
def test_arm(task_name: str, batch_size: int) -> None:
    ...
    model_start_training = 1000 # Number of generations to fill container before training the model: 1000
    freq_training = 500 # Number of generations between 2 training except the first one at model_start_training generations: 500
    # 3 ==> 2/3 of old data is kept between each training, 2 ==> 1/2 is kept, 4 ==> 3/4 is kept, 1.4 ==> 30% is kept
    buffer_size = 2 * freq_training * batch_size # Size of the buffer to store genotypes and scores:  
    num_epochs = 200 # Number of epochs at each training: 200 
    ...
    for i in tqdm.tqdm(range(num_iterations)):
        ...
        if i <= model_start_training:
            ...
            replay_buffer.add_transitions_implicit(
        ...
        # elif i != 0 and i > model_start_training and i != (freq_training - to_predict) and (i + to_predict) % freq_training == 0:
            ...
            # print("OT_model_predictions: \n", OT_model_predictions)
            # print("OT_function_predictions: \n", OT_function_predictions)


            # ### Predicting scores for one batch with the model from full repertoire 
            # solution_function, solution_model = map_elites.predict(
            #     repertoire=repertoire,
            #     batch_size=batch_size
            # )

            # ### Keeping track of the predictions for the given batch of genotype for both scoring function and model
            # OT_model_predictions.append(solution_model[1])
            # OT_function_predictions.append(solution_function[1])
            # print("OT_model_predictions: \n", OT_model_predictions)
            # print("OT_function_predictions: \n", OT_function_predictions)
    ...
    env_steps = jnp.arange(num_iterations/log_period) * batch_size * log_period # WITH SAMPLING
