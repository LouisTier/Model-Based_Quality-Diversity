"""
This file contains util functions and a class to define a repertoire, used to store individuals in the MAP-Elites
...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    Args:
        ...
        fitnesses_var: an array that contains the fitness uncertainty  of solutions in 
            each cell of the repertoire, ordered by centroids. The array shape is (num_centroids,).
        descriptors_var: an array that contains the descriptors uncertainty  of solutions in 
            each cell of the repertoire, ordered by centroids. The array shape is (num_centroids,).
    ...
    def save(self, path: str = "./") -> None:
        ...
        jnp.save(path + "fitnesses_var.npy", self.fitnesses_var)
        jnp.save(path + "descriptors_var.npy", self.descriptors_var)
    ...
    def load(cls, reconstruction_fn: Callable, path: str = "./") -> MapElitesRepertoire:
        ...
        fitnesses_var = jnp.load(path + "fitnesses_var.npy")
        descriptors_var = jnp.load(path + "descriptors_var.npy")
    ...
    def add(
        ...
        batch_of_descriptors_var: Descriptor,
        batch_of_fitnesses_var: Fitness,
        ...
        Args:
            ...
            batch_of_descriptors_var: an array that contains the descriptors uncertainty of the
                aforementioned genotypes. Its shape is (batch_size,)
            batch_of_fitnesses_var: an array that contains the fitnesses uncertainty of the
                aforementioned genotypes. Its shape is (batch_size,)
        ...
        batch_of_descriptors_var = jnp.expand_dims(batch_of_descriptors_var, axis=-1)
        batch_of_descriptors_var = jnp.reshape(batch_of_descriptors_var, (batch_of_genotypes.shape[0], 1))
        batch_of_fitnesses_var = jnp.expand_dims(batch_of_fitnesses_var, axis=-1)
        batch_of_fitnesses_var = jnp.reshape(batch_of_fitnesses_var, (batch_of_genotypes.shape[0], 1))
        ...
        batch_of_indices = jnp.where(
            addition_condition, x=batch_of_indices, y=num_centroids # Assign x if addition_condition is true, y if False
        )
        ...
        new_fitnesses_var = self.fitnesses_var.at[batch_of_indices.squeeze(axis=-1)].set(
            batch_of_fitnesses_var.squeeze(axis=-1)
        )
        new_descriptors_var = self.descriptors_var.at[batch_of_indices.squeeze(axis=-1)].set(
            batch_of_descriptors_var.squeeze(axis=-1)
        )
        ...
        return MapElitesRepertoire(
            ...
            fitnesses_var=new_fitnesses_var,
            descriptors_var=new_descriptors_var
        )
    ...
    def init(
        ...
        Args:
            ...
            descriptors: descriptors of the initial genotypes
                of shape (batch_size, num_descriptors)
            fitnesses_var: fitness uncertainty of the initial genotypes of shape (batch_size,)
            descriptors_var: descriptors uncertainty of the initial genotypes of shape (batch_size,)
        ...
        new_repertoire = repertoire.add(genotypes, descriptors, fitnesses, descriptors_var, fitnesses_var, extra_scores)
    ...
    def init_default(
        ...
        # default fitness variance is -inf
        default_fitnesses_var = -jnp.inf * jnp.ones(shape=num_centroids)

        # default descriptors variance is -inf
        default_descriptors_var = -jnp.inf * jnp.ones(shape=num_centroids)
        ...
        return cls(
            ...
            centroids=centroids,
            fitnesses_var=default_fitnesses_var,
            descriptors_var=default_descriptors_var
        )
    ...
    def empty(self) -> MapElitesRepertoire:
        ...
        new_fitnesses_var = jnp.full_like(self.fitnesses_var, -jnp.inf)
        new_descriptors_var = jnp.full_like(self.descriptors_var, -jnp.inf)
