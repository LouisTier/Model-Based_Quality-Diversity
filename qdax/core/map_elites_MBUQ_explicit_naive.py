...
class MAPElites:
    ...
    def train_model_v2(
        self,
        model_dict,
        training_set: Tuple[Genotype, Fitness, Descriptor, Fitness, Descriptor],
        num_epochs: int = 300,
        batch_size: int = 512,
        l2_rate: float = 0.000001,
    ):
        ...
        fitness_var_train = training_set[3].reshape((-1, 1))
        descriptors_var_train = training_set[4].reshape((-1, 1))
        ...
        for epoch in range(num_epochs):
            ...
            (model_dict, train_losses, mse_losses, genotypes_batches, fitnesses_batches, descriptors_batches, fitnesses_var_batches, descriptors_var_batches, batch_num, l2_rate), _ = jax.lax.scan(
                self.scan_looping_batches, # Function to scan
                (model_dict, train_losses, mse_losses, genotypes_batches, fitnesses_batches, descriptors_batches, fitnesses_var_batches, descriptors_var_batches, batch_num, l2_rate), # Input args of the function
    ...
    def looping_batches(
        ...
        function_predictions = jnp.concatenate((batch_fitness, batch_descriptors, batch_fitness_var, batch_descriptors_var), axis=1)
    ...
    def scan_looping_batches(
        ...
        model_dict, train_losses, mse_losses, genotypes_train, fitness_train, descriptors_train, fitness_var_train, descriptors_var_train, batch_num, l2_rate = carry
        ...
        return (model_dict, train_losses, mse_losses, genotypes_train, fitness_train, descriptors_train, fitness_var_train, descriptors_var_train, batch_num, l2_rate), None
    ...
    def check_model(
            self,
            model_dict,
            validation_set: Tuple[Genotype, Fitness, Descriptor, Fitness, Descriptor],
            batch_size: int = 512,
    ): #-> Genotype:
        """
        ...
        fitness_var_val = validation_set[3]
        descriptors_var_val = validation_set[4]
        ...
        descriptors_val_model = preds_model[:, 1:3]
        fitness_var_val_model = preds_model[:, 3:4]
        descriptors_var_val_model = preds_model[:, 4:5]
        ...
        fitness_var_val = fitness_var_val[0:end_idx].reshape((-1,1))
        descriptors_var_val = descriptors_var_val[0:end_idx].reshape((-1,1))

        # Concatenate the predictions of all batches from the scoring function
        preds_func = jnp.concatenate((fitness_val, descriptors_val, fitness_var_val, descriptors_var_val), axis=1)
    ...
    def model_batch_predict(
        ...
        descriptors_model = preds_model[:, 1:3]
        fitness_var_model = preds_model[:, 3:4]
        descriptors_var_model = preds_model[:, 4:5]
