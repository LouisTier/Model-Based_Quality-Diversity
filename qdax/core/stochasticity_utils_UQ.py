"""
This work is based on the UQD framework: https://github.com/adaptive-intelligent-robotics/Uncertain_Quality_Diversity
...
This code has been slightly modified and adapted by Louis BERTHIER as part of his Independent Study Option and 
Individual Research Project at Imperial College London. 
...
# from .containers.mapelites_repertoire import MapElitesRepertoire
from .containers.mapelites_repertoire_UQ import MapElitesRepertoire
...
def sampling_both(
    ...
    # evaluate
    sample_scoring_fn = jax.vmap(scoring_fn, (None, 0), 1)
    all_fitnesses_arm, all_fitnesses_noisy_arm, all_descriptors_arm, all_descriptors_noisy_arm, all_extra_scores, _ = sample_scoring_fn(
        policies_params, keys
    )

    # compute new results
    if use_median:
        descriptors_arm = jnp.median(all_descriptors_arm, axis=1)
        fitnesses_arm = jnp.median(all_fitnesses_arm, axis=1)
        descriptors_noisy_arm = jnp.median(all_descriptors_noisy_arm, axis=1)
        fitnesses_noisy_arm = jnp.median(all_fitnesses_noisy_arm, axis=1)
    else:
        descriptors_arm = jnp.average(all_descriptors_arm, axis=1)
        fitnesses_arm = jnp.average(all_fitnesses_arm, axis=1)
        descriptors_noisy_arm = jnp.average(all_descriptors_noisy_arm, axis=1)
        fitnesses_noisy_arm = jnp.average(all_fitnesses_noisy_arm, axis=1)

    # compute variance
    descriptors_var_arm = jnp.mean(jnp.nanstd(all_descriptors_arm, axis=1), axis=1)
    fitnesses_var_arm = jnp.nanstd(all_fitnesses_arm, axis=1)
    descriptors_var_noisy_arm = jnp.mean(jnp.nanstd(all_descriptors_noisy_arm, axis=1), axis=1)
    fitnesses_var_noisy_arm = jnp.nanstd(all_fitnesses_noisy_arm, axis=1)

    # extract extra scores and add number of evaluations to it
    extra_scores = extra_scores_extractor(all_extra_scores, num_samples)
    extra_scores["num_evaluations"] = jnp.full(fitnesses_arm.shape[0], num_samples)
    ...
    return (
        ...
        descriptors_var_arm,
        fitnesses_noisy_arm,
        fitnesses_var_noisy_arm,
        descriptors_noisy_arm,
        descriptors_var_noisy_arm,
        extra_scores,
        random_key,
    )
...
def reevaluation_function(
    ...
    fit_reeval_repertoire = fit_reeval_repertoire.add(
        ...
        fitnesses,
        descriptors_var, # Not sure but thi repertoire is unused in the main script
        fitnesses_var, # Not sure but thi repertoire is unused in the main script
        extra_scores,
    )
...
def reevaluation_function_both(
    repertoire: MapElitesRepertoire,
    random_key: RNGKey,
    metric_repertoire: MapElitesRepertoire,
    scoring_fn: Callable[
    ...
    # Set -inf fitness for all unexisting indivs
    fitnesses = jnp.where(repertoire.fitnesses == -jnp.inf, -jnp.inf, fitnesses_noisy_arm)
    fitnesses_var = jnp.where(repertoire.fitnesses == -jnp.inf, -jnp.inf, fitnesses_var_noisy_arm)
    descriptors_var = jnp.where(
        repertoire.fitnesses == -jnp.inf, -jnp.inf, descriptors_var_noisy_arm
    )
