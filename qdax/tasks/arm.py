...
def both_scoring_function(
    ...
    # Evaluate
    fitnesses_arm, descriptors_arm = jax.vmap(arm)(params)

    # Add noise to the fitnesses and descriptors
    fitnesses_noisy_arm = (
        fitnesses_arm + jax.random.normal(f_subkey, shape=fitnesses_arm.shape) * fit_variance
    )
    descriptors_noisy_arm = (
        descriptors_arm
        + jax.random.normal(d_subkey, shape=descriptors_arm.shape) * desc_variance
    )
