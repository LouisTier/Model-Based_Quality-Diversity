...
def default_qd_metrics_UQ(repertoire: MapElitesRepertoireUQ, qd_offset: float) -> Metrics:
    """Compute the usual QD metrics that one can retrieve
    from a MAP Elites repertoire.
    ...
    Returns:
        a dictionary containing the QD score (sum of fitnesses
            modified to be all positive), the max fitness of the
            repertoire, the coverage (number of niche filled in
            the repertoire), the UFS (sum of fitness uncertainty)
            and the UBS (sum of BD uncertainty).
    """
    ...
    coverage = 100 * jnp.mean(1.0 - repertoire_empty)
    max_fitness = jnp.max(repertoire.fitnesses)
    std_fitness = jnp.sum(repertoire.fitnesses_var, where=~repertoire_empty)
    std_bd = jnp.sum(repertoire.descriptors_var, where=~repertoire_empty)

    return {"qd_score": qd_score, "max_fitness": max_fitness, "coverage": coverage, "std_fitness": std_fitness, "std_bd": std_bd}
