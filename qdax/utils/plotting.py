...
from qdax.core.containers.mapelites_repertoire_UQ import MapElitesRepertoire as MapElitesRepertoireUQ
...
def plot_2d_map_elites_repertoire(
    ...
    # aesthetic
    ax.set_xlabel("Behavior Dimension 1: x")
    ax.set_ylabel("Behavior Dimension 2: y")
...
def plot_map_elites_results_UQ(
    env_steps: jnp.ndarray,
    metrics: Dict,
    repertoire: MapElitesRepertoireUQ,
    min_bd: jnp.ndarray,
    max_bd: jnp.ndarray,
) -> Tuple[Optional[Figure], Axes]:
    """Plots three usual QD metrics, namely the coverage, the maximum fitness
    ...
    axes[3].plot(env_steps, metrics["std_fitness"])
    axes[3].set_xlabel("Environment steps")
    axes[3].set_ylabel("Sum of the std of the fitnesses")
    axes[3].set_title("Fitnesses' stds evolution during training")
    axes[3].set_aspect(0.95 / axes[3].get_data_ratio(), adjustable="box")

    axes[4].plot(env_steps, metrics["std_bd"])
    axes[4].set_xlabel("Environment steps")
    axes[4].set_ylabel("Sum of the std of the BDs")
    axes[4].set_title("BDs' stds evolution during training")
    axes[4].set_aspect(0.95 / axes[4].get_data_ratio(), adjustable="box")
